    ...
    internal class Menus
    {
        public static void Login()
        {
            using (var db = new WebShopFoodContext())
            {
                while (true)
                {
                    ...
                    {
                        Console.WriteLine("There are no users, please create a new user");
                        Admin.AddCostumer();
                        var costumerList = Admin.DisplayCostumerNames(db);
                        string costumerNew = Admin.FindCostumerId(db, costumerList.First());
        ...
        public static void ShoppingCartView(Costumer costumer)
        {
            using (var db = new WebShopFoodContext())
            {
                ...
                var tP1 = (from sIL in shoppingItemsList
                          select sIL.Price * sIL.Quantity).Sum();
                Console.WriteLine("Total price without transportcost cost: " + (tP1 * 1.25));
                    ...
                    case '1':
                        ...
                        while (true)
                        {
                            var shoppingItemsCheck3 = (from ShoppingItem in db.ShoppingItems
                                                       ...
                                                       select ShoppingItem).ToList();
                            Console.Clear();
                            Console.WriteLine(productToAlterAmount + " : "+ shoppingItemsCheck3[0].Quantity);
